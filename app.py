...
def scrape_sanmin(isbn):
    ...
    try:
        ...
        publisher_tag = item.select_one('.Publisher')
    ...
    try:
        ...
        if res.status_code == 200:
            ...
            if data.get('totalItems', 0) > 0:
                ...
                pd_date = v.get('publishedDate', '')
                y = pd_date.split('-')[0] if pd_date else None
                m = pd_date.split('-')[1] if len(pd_date.split('-')) > 1 else None
        ...
        if response.status_code == 200:
            ...
            if 'items' in data:
                for item in data['items']:
                    ...
                    pd_date = v.get('publishedDate', '')
                    year = pd_date.split('-')[0] if pd_date else ""
                    img = v.get('imageLinks', {})
