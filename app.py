...
from werkzeug.utils import secure_filename  # 用來處理上傳檔名
...
def add_book():
    if request.method == 'POST':
        try:
            # 1. 先處理圖片上傳
            cover_url = request.form.get('cover_url') # 預設拿網址欄位
            ...
            new_book = Book(
                ...
                cover_url=cover_url  # 使用處理後的圖片路徑
            )
...
def api_lookup(isbn):
    return jsonify({"error": "Use frontend lookup"}), 404
